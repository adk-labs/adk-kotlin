...
- Python reference reviewed: `../ref/adk-python/src/google/adk` (`541` Python files)
- Java reference reviewed: `../ref/adk-java/core/src/main/java/com/google/adk` (`196` Java files)
...
| App container and root agent model | `apps/app.py`, `_configs.py`, compaction, summarizer hooks, resumability config | `apps/App.java`, cache/compaction config | `AdkApp` with root agent, global instruction, transfer tree helpers, events compaction config, resumability config | Partial | Add app-level context cache, resumability runtime, service composition, richer app metadata, and first-party summarizer implementations | P0 |
...
| Model/provider layer | Multiple provider implementations, registry, request/response config, interactions utilities | Model abstraction, registry, Chat Completions HTTP client | `Model`, `BaseLlm`, Gemini/Claude/Apigee models, explicit transport registry, OpenAI-compatible chat-completions HTTP transport, capabilities, generate config | Partial | Add streaming Chat Completions, first-party Gemini/Claude transports/connections, response metadata propagation, Anthropic/interactions conversion edge cases, empty-candidate success handling, and provider-specific utilities | P0 |
| Tool base runtime | Base tool classes, toolsets, tool config, confirmations, agent tools | Base tools, transfer/set-model-response, MCP/computer-use/retrieval | Tool schema, function tools, toolsets, confirmations, auth hooks, agent tool, load/preload memory, load artifacts, skill toolset | Partial | Add official function declaration conversion depth, long-running function tools, MCP/OpenAPI/retrieval/computer-use toolsets, AgentTool content-part/code-execution output preservation, and richer tool config | P0 |
...
| A2A support | Full `a2a` package, converters, executor, utils, RemoteA2AAgent updates | Separate A2A module in official repo | None | Missing | Add A2A event/request converters, agent card conversion, client/server integration, and RemoteA2AAgent after event model and non-LLM orchestration are in place | P3 |
...
| CLI, dev server, browser UI, conformance | Large CLI/dev/browser/conformance surface | Separate dev/web modules outside core | CLI runner, web server, SSE, recordings schema/plugin | Partial | Add official CLI command parity, browser UI assets, eval/conformance commands, replay/test tooling, service registry, and latest deploy/click command behavior | P3 |
| Evaluation and optimization | Large evaluation/optimizer/simulator stack, live evaluation generator | Samples and evaluation-adjacent utilities | None | Missing | Add eval services, evaluation generator, live evaluation mode, local eval service, simulators, metrics, and optimization stack after provider/session parity improves | P3 |
| Integrations | Slack, LangChain, CrewAI, agent registry, skill registry, GCP services | Some integration-oriented tooling | None | Missing | Add GCP skill registry/search, agent registry, agent identity, parameter/secret/firestore integrations, VMAAS, Slack, LangChain, and CrewAI after stable tool/provider/plugin APIs exist | P3 |
| Telemetry and tracing | Dedicated telemetry package, Google Cloud telemetry helpers | Dedicated telemetry package | None | Missing | Add tracing abstraction, Google Cloud exporter/attributes, span export, request/response metadata capture, compaction telemetry, and plugin/runner integration | P2 |
| Skills, platform, features, misc utils | Separate skills/platform/features/errors/utils packages, skill zip/utils | `skills` package, utility packages | `SkillSource`, `Frontmatter`, in-memory/local skill sources, `SkillToolset` list/load/resource tools; minimal inline platform/utils and typed errors | Partial | Add skill registry integrations/search, zip skill loading, binary resource handling, script execution, allowed-tools resolution, platform/time/uuid shims, feature registry, utility packages, and official error hierarchy when concrete modules need them | P2 |
...
## Latest Ref Sync Audit Notes

- `adk-java` v1.3.x added `ChatCompletionsHttpClient`; Kotlin now has the
  non-streaming HTTP path, but streaming SSE remains a tracked model-layer gap.
- `adk-python` added `apps/_configs.py`, app compaction changes, and
  resumability config. Kotlin now has config objects and post-run compaction,
  but not resumability runtime or first-party LLM event summarizers.
- `adk-python` added `integrations/skill_registry/gcp_skill_registry.py` and
  `skills/_utils.py`. Kotlin has local/in-memory skills and runtime skill
  tools, but not GCP registry/search, zip loading, binary resource handling, or
  script execution.
- `adk-python` added live evaluation support in `evaluation_generator.py` and
  `local_eval_service.py`; Kotlin still has no evaluation stack.
- `adk-python` updated A2A converters and `RemoteA2AAgent`; Kotlin still has no
  A2A module.
- `adk-python` updated Google Cloud telemetry helpers; Kotlin still has no
  telemetry/tracing package.
- Provider edge-case parity is still incomplete: Anthropic tool-result string
  handling, interactions result pass-through, empty-candidate success responses,
  and AgentTool code-execution/executable-code output preservation are now
